...
//let determinantValue = ref(Number())
...
function calculdeterminant(matrix) {
  ...
  for (let j = 0; j < n; j++) {
    // Calculate the minor matrix by removing the first row and the j-th column
    const minorMatrix = matrix.slice(1).map(row => row.slice(0, j).concat(row.slice(j + 1)));
...
const remplirauto = (()=>{
  for(let i = 0 ; i<tabledimension.value ; i++){
    for(let j = 0; j<tabledimension.value; j++){
      datainput.value[i][j]=Math.floor(Math.random() * (100 - (-100) + 1) ) + (-100);
    }
  }
})
  ...
  <div id="viewport">
    ...
    <div id="content">
      ...
      <div>
        ...
        <input v-model="dimension" class="choixInput" type="number" min="2" max="9"/>
        <button @click="createTable" class="btn btn-info matriceBouton">Ok</button>
      </div>
        <div class="d-flex matriceTable">
            <table>
              <body>
                <tr v-for="(n,indexp) in tabledimension" :key="indexp" >
                  <td v-for="(n,index) in tabledimension" :key="index"><input v-model="datainput[indexp][index]" type="number" class="matriceInput"/></td>
                </tr>
              </body>
            </table>
        </div>
        <button v-if="tabledimension" @click="remplirauto" class="btn btn-secondary replirauto">Remplir auto</button>
        <button v-if="tabledimension" @click="generermatrice" class="btn btn-primary">Générer</button>
        <div class="determinant">
          <label v-if="determinant" for="">Le détérminant est:<span class="font-weight-bold ml-2">{{ determinant }}</span></label>
        </div>
